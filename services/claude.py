...
- 流量异动、渠道、转化漏斗、活动承接
- **品类相关**：仅"流量转化路径异动"（如某品类入口 UV/PV 涨跌、搜索/金刚位/信息流的占比变化）放这里

### 板块 5：履约流程（汇报人：郑文浩）
- 履约链路、质检、服务商、上门数据

### 板块 6：品类运营（汇报人：发展品类-熊曼佚 / 孵化品类-陈俊）
- 各品类的估价、下单、成交、价格策略、新品节奏。**子标题先按"发展品类"和"孵化品类"区分**
- **孵化品类固定是这 5 个**：台球杆、公路车、名酒、盲盒、电动车。讨论这 5 个品类的内容必须放在 `#### 孵化品类` 下
- 其余品类（手机、组装机、内存条、显卡、相机、无人机、智能数码、电脑办公等）放在 `#### 发展品类` 下
- **品类相关**：本品类的"估价完成率/下单率/成交率/客单价/价格策略/新品节奏"放这里
...
**板块 4 vs 板块 6 的判定准则（关键）**：
- 同一个品类的内容可能拆到两个板块里，**按"事的性质"判定**：
  - "XX 品类流量上涨/下跌、入口占比变化、搜索/金刚位流量份额" → 板块 4（增长）
  - "XX 品类估价完成率、下单率、成交率、客单价、价格策略、新品上线" → 板块 6（品类运营）
- 例：内存条搜索流量涨 44.5% → 板块 4；组装机精准估价转化下滑 → 板块 6
- 例：无人机估价 UV 下降 12% → 板块 4；无人机价格调整 → 板块 6

**关键约束**：
- 即使某个板块本周没汇报内容，也要保留板块标题，正文写"本周无汇报"
- 不要把内容串到错的板块里：例如"豁免质检"必须在板块 5（履约流程），不能放在板块 7
- 板块 6（品类运营）的子议题先用 `#### 发展品类` 和 `#### 孵化品类` 分组，再列具体品类
- 如果某个发言人在自己负责的板块里讲到了别人板块的事，按"事的归属"而非"说话人"决定放哪个板块
- 板块标题原样保留"汇报人：xxx"信息，方便事后追溯

**⚠️ 7 大板块只是"二、关键讨论点"内部的结构，绝不取代整体输出格式**：
- 必须按 output_format.md 完整输出 6 大节：会议目标 / 关键讨论点（=7 板块）/ 决策事项 / 待办事项 / 未解决问题 / 备注
- "三、决策事项"和"四、待办事项"必须是**全局汇总表格**，把所有板块里讨论出来的决策/待办抽取到表里，**不允许省略**
- 决策事项表至少包含本场会议明确拍板的事项；待办事项表至少包含明确指派了负责人和时间的任务
- 如果板块里讨论出了决策/待办，板块内简述结论即可，**详细信息必须**进对应的全局表格
- 板块 1（上周待办回顾）的内容是上周遗留的，**不要**重复进本场的"四、待办事项"
"""
